...
@section('content')
    <div style="margin: 20px;">
        <h4>
            Modifier un cours </h4>
        <div style="border: solid 1px grey; padding: 20px;">
            <form method="POST" action="{{ route("admin.cours.update", [$cour->id]) }}" enctype="multipart/form-data">
                @method('PUT')
                @csrf
                    ...
                    @if (Auth::user()->isAdmin())
                        ...
                        <select class=" select2" style="width: 100%; padding: 4px;" name="enseignants[]" id="enseignants" multiple>
                            @foreach($enseignants as $id => $enseignants)
                                <option value="{{ $id }}" {{ (in_array($id, old('enseignants', [])) || $cour->enseignants->contains($id)) ? 'selected' : '' }}>{{ $enseignants }}</option>
                            @endforeach
                        </select>
                ...
                <div >
                    ...
                    <select class=" select2" style="width: 100%; padding: 4px;" name="categories_id" id="categories_id">
                        @foreach($category as $id => $category)
                            <option value="{{ $id }}" {{ ($cour->category ? $cour->category->id : old('categories_id')) == $id ? 'selected' : '' }}>{{ $category }}</option>
                        @endforeach
                    </select>
                ...
                <div class="{{ $errors->has('titre') ? 'has-error' : '' }}">
                    ...
                    <input style="width: 100%; padding: 4px;" type="text" name="titre" id="titre" value="{{ old('titre', $cour->titre) }}" required>
                    @if($errors->has('titre'))
                ...
                <div class="{{ $errors->has('description') ? 'has-error' : '' }}">
                    ...
                    <textarea style="width: 100%; padding: 4px;" name="description" id="description">{{ old('description', $cour->description) }}</textarea>
