...
@section('content')
    <div style="margin: 20px;">
        <h4>
            Ajouter une leçon </h4>
        <div style="border: solid 1px grey; padding: 20px;">
                    <form method="POST" action="{{ route("admin.lecons.store") }}" enctype="multipart/form-data">
                        @csrf
                        <div class="{{ $errors->has('chapitre') ? 'has-error' : '' }}">
                            ...
                            <select class=" select2" style="width: 100%; padding: 4px;" name="chapitre_id" id="chapitre_id">
                                @foreach($chapitres as $id => $chapitre)
                                    <option value="{{ $id }}" {{ old('chapitre_id') == $id ? 'selected' : '' }}>{{ $chapitre }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="{{ $errors->has('texte_long') ? 'has-error' : '' }}">
                            ...
                            <textarea style="width: 100%; padding: 4px;" name="texte_long" id="texte_long">{{ old('texte_long') }}</textarea>
