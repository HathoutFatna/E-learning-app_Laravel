    ...
    <div style="border: solid 1px grey; padding: 20px;">
        <form method="POST" action="{{ route("admin.quizzes.update", [$quiz->id]) }}" enctype="multipart/form-data">
            ...
            <div class=" {{ $errors->has('questions') ? 'has-error' : '' }}">
                ...
                <select class="select2" style="width: 100%; padding: 4px;"  name="questions[]" id="questions" multiple required>
                    @foreach($questions as $id => $questions)
                        <option value="{{ $id }}" {{ (in_array($id, old('questions', [])) || $quiz->questions->contains($id)) ? 'selected' : '' }}>{{ $questions }}</option>
                    @endforeach
                </select>
            ...
            <div class="{{ $errors->has('cours') ? 'has-error' : '' }}">
                ...
                <select class=" select2" style="width: 100%; padding: 4px;"  name="cours_id" id="cours_id">
                    @foreach($cours as $id => $cours)
                        <option value="{{ $id }}" {{ ($quiz->cours ? $quiz->cours->id : old('cours_id')) == $id ? 'selected' : '' }}>{{ $cours }}</option>
                    @endforeach
                </select>
            ...
            <div class=" {{ $errors->has('titre') ? 'has-error' : '' }}">
                ...
                <input style="width: 100%; padding: 4px;"  type="text" name="titre" id="titre" value="{{ old('titre', $quiz->titre) }}" required>
                @if($errors->has('titre'))
            ...
            <div class="{{ $errors->has('description') ? 'has-error' : '' }}">
                ...
                <textarea style="width: 100%; padding: 4px;"  name="description" id="description">{{ old('description', $quiz->description) }}</textarea>
