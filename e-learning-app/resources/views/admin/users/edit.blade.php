    ...
    <div style="border: solid 1px grey; padding: 20px;">
        <form method="POST" action="{{ route("admin.users.update", [$user->id]) }}" enctype="multipart/form-data">
            ...
            <div class="{{ $errors->has('name') ? 'has-error' : '' }}">
                ...
                <input  style="width: 100%; padding: 4px;"type="text" name="name" id="name" value="{{ old('name', $user->name) }}" required>
                @if($errors->has('name'))
            ...
            <div class="{{ $errors->has('email') ? 'has-error' : '' }}">
                ...
                <input  style="width: 100%; padding: 4px;" type="text" name="email" id="email" value="{{ old('email', $user->email) }}" required>
                @if($errors->has('email'))
